...
## Recalibration wizard: structured status contract (2026-08-06)

`RecalibrationWizardPanel`'s SENSOR_RUNNING stage no longer scans
`handle_gcode_response()`'s raw text for `"z_offset:"`/`"PR_ERR_CODE"`. It now subscribes
to `[z_compensate]`'s structured Klipper status object
(`calibration_id`/`calibration_state`/`calibration_z_offset`/`calibration_error`) instead —
full contract in `docs/z_compensate_status_api.md` (synced from the `ke-mainline-klipper`
repo, which owns the backend object). The ZOFFSET stage's own stock-Klipper
`PROBE_CALIBRATE` text parsing is untouched — different command, different (upstream, not
project-specific) contract.

Two new pure-logic, LVGL-free classes carry the actual decision logic, both host-testable
without a display (`make test`):

- **`ZCompensateStatusTracker`** (`src/z_compensate_status.{h,cpp}`) — validates one merged
  status snapshot against the contract and correlates a sequence of them against one
  active `Z_OFFSET_CALIBRATION` invocation via `calibration_id`. `RecalibrationWizardPanel`
  owns one (`calibration_tracker`), `begin()`s it with the pre-command baseline id in
  `start_sensor_refine()`, and feeds it every subsequent `z_compensate` snapshot from
  `consume()` via the new `handle_z_compensate_status()` method.
- **`ZOffsetConfigPersistence`** (`src/z_offset_config_persistence.{h,cpp}`) — the wizard's
  existing `printer.cfg` backup/patch/restore logic, extracted verbatim (not
  reimplemented) so it's testable without the panel around it. Extraction surfaced one
  real, pre-existing latent bug (not introduced by this change): opening a directory path
  via `std::ifstream` passes the original `if (!stream)` check but throws on the first
  actual read — fixed with a `stat()`-based regular-file guard before every read.

Both `CRTENSE_NOZZLE_CLEAR` and `Z_OFFSET_CALIBRATION` are now sent via
`ws.gcode_script(cmd, callback)` (JSON-RPC error callback wired to
`calibration_tracker.fail_command()`) instead of the old fire-and-forget
`ws.gcode_script(cmd)` — a JSON-RPC-level command failure and a structured `"error"` status
update are two independent signals of the same failure and the tracker latches on
whichever arrives first, ignoring the other.

**Known pre-existing gap, not introduced or touched by this change**: `FIRMWARE_RESTART` is
still sent fire-and-forget (`ws.gcode_script("FIRMWARE_RESTART")`, no callback) in three
places (`sensor_refine_failed()`, the Save & Restart path, and the Discard path) — this
predates this change (present in the original wizard commit) and is consistent with
Klipper actually restarting/disconnecting on this command, so there is no meaningful
success response to wait for. Flagging it here rather than silently working around it: a
`printer.cfg` write that's already succeeded is not rolled back if the *restart itself*
fails to reach the printer, which is a real (pre-existing) gap if that specific failure
mode ever matters in practice.

Tests: `tests/test_z_compensate_status.cpp` (36 cases — parsing, correlation, and proof
that legacy-shaped text embedded in an otherwise valid snapshot has zero effect, since the
parser only ever reads the four named JSON fields), `tests/test_z_offset_config_persistence.cpp`
(9 cases), `tests/test_contract_fixture.cpp` (5 cases, replays the literal fixture
generated by the Klipper-side backend), `tests/test_integration_harness.cpp` (3 cases,
full success/failure trace replay including a real `ZOffsetConfigPersistence` write) — see
`tests/fixtures/README.md` for how those fixtures get here. `make test` runs all of it.
